...
const { addUser, UserHasShop, createShop, getAllShop, checkInput, queryItems, queryShop, remItem, getUser, updateShopInfo } = require("./sql");
...
class user {
    ...
    static mainFunc = (pl, dt) => {
        ...
        if (dt == 0) {
            if (UserHasShop(pl.xuid)) {
                pl.sendForm(this.myshopForm(pl.xuid),this.myshopFunc(pl.xuid));
            } else {
    ...
    static buyFunc(tableid, select) {
        return (pl, dt) => {
            ...
            if (dt[1] == select.count) {
                remItem(tableid, select.id, 0);
            } else {
                remItem(tableid, select.id, dt[1]);
            }
        }
    }
    static myshopForm(xuid) {
        let shop = queryShop('', xuid);
        let user = getUser(xuid);
        let fm = mc.newSimpleForm();
        fm.setTitle(shop.NAME);
        fm.setContent(`
${format(lang.myshop.main.star, { star: shop.STARS })}
${format(lang.myshop.main.hmoney, { score: user.MONEY })}`);
        fm.addButton(lang.myshop.main.me);
        fm.addButton(lang.myshop.main.item);
        fm.addButton(lang.myshop.main.history);
        return fm;
    }
    static myshopFunc = (xuid) => {
        return (pl, dt) => {
            if (dt == null) return;
            let shop = queryShop('', xuid);
            let user = getUser(xuid);
            switch(dt){
                case 0:
                    ...
                    fm1.setTitle(lang.myshop.me.title);
                    fm1.addInput(lang.myshop.me.name,'',shop.NAME);
                    fm1.addInput(lang.myshop.me.desc,'',shop.DESC);
                    pl.sendForm(fm1,(pl,dt)=>{
                        if(dt==null)return;
                        if (checkInput(dt[0]) || checkInput(dt[1])) {
                            warn(lang.warn.error_input, pl, (pl) => {
                                pl.sendForm(this.myshopForm(xuid), this.myshopFunc(xuid));
                            });
                        } else {
                            updateShopInfo(dt[0],dt[1]);
                        }
                    })
                    break;
            }
